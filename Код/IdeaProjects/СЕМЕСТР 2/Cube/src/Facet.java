...
public class Facet {
    private final R3Vector[] _vertex;
    ...
    public void rotate(double ux, double uy, double uz) {
        for (R3Vector vertex : _vertex) {
            vertex.rotate(ux, uy, uz);
        }
    }

    public void ortdraw(Graphics2D g, boolean flag) {
        if (flag) {
            boolean trans = Main.Transparent.isSelected();
            if (trans || (!trans
                    && R3Vector.vect(R3Vector.toR3Vector(_vertex[0], _vertex[1]), R3Vector.toR3Vector(_vertex[1], _vertex[2])).getZ() > 0)) {
    ...
    public void perspdraw(Graphics2D g, double c) {
        ...
        boolean trans = Main.Transparent.isSelected();
        if (trans || (!trans
                && R3Vector.vect(R3Vector.toR3Vector(v0, v1), R3Vector.toR3Vector(v1, v2)).getZ() > 0)) {
            Path2D p = new Path2D.Double();
